...
function buildCheckSessionResponse() {
    // Return empty server role list to force create character flow
    // server_role_t: 1=server_info, 2=role_info(optional)
    const serverInfo = Buffer.concat([
        encodeUint32(1, 1),
    ...
    return encodeMessage(1, serverInfo);
}
...
function handleGameClient(socket) {
    ...
    socket.on('data', (data) => {
        ...
        while (buffer.length >= 4) {
            ...
            // Build response
            const respCmd = CMD_MAP[header.cmd] || header.cmd.replace(/_in$/, '_out');
...
function buildResponse(cmd, fields, socket) {
    ...
    if (cmd.includes('create_role')) {
        ...
        STATE.roles[serverId].push({roleTm, nick, gender, uid: STATE.nextUid++, level: 1});
        console.log(`[ROLE] Created: ${nick} (role_tm: ${roleTm})`);
        // Fall through to login response
    ...
    const genericEmptyList = [
        'get_item', 'get_bag_mon', 'get_friend_list', 'get_mail_list',
